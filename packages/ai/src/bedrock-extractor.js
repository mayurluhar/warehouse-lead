...
export class BedrockLeadExtractor {
    ...
    async extract(title, text) {
        ...
        if (this.client) {
            try {
                ...
                console.warn(`[Bedrock Ingestion] Bedrock invocation returned error: ${err.message}. Falling back to Rule-based NLP extraction.`);
            }
        }
    ...
    async extractWithBedrockConverse(documentText) {
        if (!this.client)
            return null;
