...
/**
 * A fixed bank of realistic warehouse signals used to exercise the pipeline
 * without depending on what Google News happens to be serving.
 *
 * The last entry is deliberately irrelevant — a stock-market story about
 * residential real estate — so every run also proves the relevance classifier
 * still rejects market commentary that mentions real estate but contains no
 * occupier requirement.
 */
export const BENCHMARK_SIGNALS: Omit<SourceDocument, 'id' | 'contentHash' | 'fetchedAt'>[] = [
  {
...
/**
 * Serves the benchmark bank through the same Port as a live connector, so the
 * ingestion pipeline cannot tell the difference between sample and real input.
 */
export class SampleSignalBank implements IDocumentSource {
  public readonly sourceName = "benchmark_signal_bank";
  ...
  public async fetchDocuments(): Promise<SourceDocument[]> {
    return BENCHMARK_SIGNALS.map((signal, idx) => ({
      ...signal,
      id: `sample_${idx + 1}_${Date.now()}`,
      contentHash: this.normalizer.computeContentHash(`${signal.title}\n${signal.cleanText}`),
